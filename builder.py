...
def build(version=None,
    ...
    for target, path in targets.items():
        ...
        if static:
            build_command += ldflags.format(version, datetime.utcnow().strftime("%Y%m%d%H%M"), get_current_branch(), get_current_commit())
        else:
            build_command += ldflags.format(version, datetime.utcnow().strftime("%Y%m%d%H%M"), get_current_branch(), get_current_commit())
...
if __name__ == '__main__':
    ...
    parser.add_argument('--containerize',
                        action='store_true',
                        help='Build docker container of build artifact')
