...
func Kubectl(config types.Config, kubectlOpts *types.KubectlOpts) {
	...
	node := integration.GetFirstAccessibleNode(config.Ssh, group.Nodes, kubectlOpts.Debug)
	...
	if err != nil {
		integration.PrettyPrintErr("Error performing kubectl command %s: %s", kubectlOpts.Command, err)
	} else {
